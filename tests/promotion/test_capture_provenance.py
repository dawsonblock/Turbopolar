...
from rfsn_v11.promotion.schema import GitTreeState
...
class TestCaptureProvenance(unittest.TestCase):
    ...
    def test_capture_provenance_populates_required_fields(self):
        """P0: capture_provenance() must populate kernel_binding_hash and execution_mode."""
        config = TurboPolarConfig(
            num_q_heads=8,
            num_kv_heads=8,
            head_dim=128,  # Required by TurboPolar fused MLX path
            block_size=64,
            execution_mode=ExecutionMode.METAL_STRICT,
        )

        # Create a minimal prompt suite file for the test
        with mock.patch("rfsn_v11.promotion.provenance._file_sha256") as mock_file_hash, \
             mock.patch("rfsn_v11.promotion.provenance._dir_sha256") as mock_dir_hash, \
             mock.patch("rfsn_v11.promotion.provenance._run") as mock_run, \
             mock.patch("rfsn_v11.promotion.provenance.platform") as mock_platform, \
             mock.patch("rfsn_v11.promotion.provenance.uuid") as mock_uuid, \
             mock.patch("rfsn_v11.promotion.provenance.datetime") as mock_datetime:
            ...
            # Mock git commands - use tuple(cmd) for dict key lookup
            cmd_responses = {
            ...
            mock_run.side_effect = lambda cmd: cmd_responses.get(tuple(cmd), "")

            # Mock file/dir hashes
            ...
            mock_datetime.now.return_value.isoformat.return_value = "2026-01-01T00:00:00Z"
        ...
        # P0: Verify required fields are populated (not empty)
        self.assertTrue(
    ...
    def test_captured_provenance_passes_validation(self):
        """P0: Captured provenance must pass platform and execution validation."""
        config = TurboPolarConfig(
            num_q_heads=8,
            num_kv_heads=8,
            head_dim=128,  # Required by TurboPolar fused MLX path
        ...
        with mock.patch("rfsn_v11.promotion.provenance._file_sha256") as mock_file_hash, \
            ...
            # Mock git commands - simulate clean tree
            cmd_responses = {
                ("git", "rev-parse", "HEAD"): "a" * 40,
            ...
            # Mock file/dir hashes with full 64-char hashes
            mock_file_hash.return_value = "f" * 64
            mock_dir_hash.return_value = "d" * 64
        ...
        # Validate using the evidence module validators
        from rfsn_v11.evidence.provenance import ProvenanceEvidence

        # Convert to ProvenanceEvidence for validation
        pv = ProvenanceEvidence(
            ...
            trace_validation_mode=provenance.turbopolar_config.get("trace_validation_mode", ""),
            page_capacity=provenance.turbopolar_config.get("page_capacity_blocks", 16),
            block_size=provenance.turbopolar_config.get("block_size", 64),
            k_bit_widths=f"{provenance.turbopolar_config.get('k_angle_bits_level1', 8)}/{provenance.turbopolar_config.get('k_angle_bits_deep', 8)}",
            v_bit_width=provenance.turbopolar_config.get("v_bits", 8),
            v_group_size=32,
            num_q_heads=provenance.turbopolar_config.get("num_q_heads", 8),
            num_kv_heads=provenance.turbopolar_config.get("num_kv_heads", 8),
            head_dim=provenance.turbopolar_config.get("head_dim", 64),
            attention_scale=provenance.turbopolar_config.get("attention_scale", 1.0),
        ...
        self.assertEqual(
            immutable_errors, [],
            f"Provenance failed immutable field validation: {immutable_errors}"
        )
        ...
        self.assertEqual(
            execution_errors, [],
            f"Provenance failed execution mode validation: {execution_errors}"
        )

    def test_captured_provenance_round_trips(self):
        """P0: Captured provenance must round-trip through serialization without data loss."""
        ...
        # P0: Verify critical fields survive round-trip
        self.assertEqual(restored.kernel_binding_hash, provenance.kernel_binding_hash)
        self.assertEqual(restored.execution_mode, provenance.execution_mode)
        self.assertEqual(restored.run_id, provenance.run_id)
        self.assertEqual(restored.git_commit, provenance.git_commit)
        self.assertEqual(restored.turbopolar_config_hash, provenance.turbopolar_config_hash)
