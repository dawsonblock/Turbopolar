...
class TestTurboPolarOnlineAttention(unittest.TestCase):
    ...
    def test_phase_8_dense_v_gate(self):
        ...
        ref_output = mx.sum(
            weights[:, :, :, None] * v_dense.reshape(B, H, S * L, D), axis=-2
        )
    ...
    def test_phase_9_quant_v_gate(self):
        ...
        weights = mx.softmax(scores, axis=-1)
        ref_output = mx.sum(
            weights[:, :, :, None] * v_dequant.reshape(B, H, S * L, D), axis=-2
        )
        ...
        for T in (4096, 8192):
            with self.subTest(T=T):
                ...
                self.assertLessEqual(
                    max_err, allowed_err, f"max_err={max_err} at T={T}"
                )
    ...
    def test_high_quality_config_online_attention(self):
        ...
        blocks = [
            polar_encoder.encode_block(k_blocked[:, :, s, :, :]) for s in range(S)
        ]
    ...
    def test_phase_9_dense_tail_gate(self):
        ...
        scores = (
            mx.sum(q[:, :, None, :] * full_k, axis=-1) * self.config.attention_scale
        )
        ...
        max_err = np.max(np.abs(ref_np - gpu_np))
        self.assertEqual(
            trace["kernel_name"], "tqpolar_online_attention_quant_v_dense_tail"
        )
        self.assertGreaterEqual(cosine, 0.999)
        self.assertLessEqual(max_err, 1e-3)
